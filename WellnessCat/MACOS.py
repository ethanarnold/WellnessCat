...
class Pet():
   def __init__(self):
       ...
       self.label.configure(image=self.img, bg='white')  # Set label background color
       self.label.pack()
       ...
       self.color_index = 0
       self.window.after(20000, self.display_message) #change the time when actually using, set to several seconds now for demo
   ...
   def posture_reminder(self):
        if not self.is_stopped:
            ...
            message = "Posture check! :3"
            label = tk.Label(message_window, text=message, font=("Comic Sans MS", 12), fg='white')
            label.pack(pady=20)


            self.window.after(20000, self.posture_reminder) #change the time when actually using, set to several seconds now for demo
